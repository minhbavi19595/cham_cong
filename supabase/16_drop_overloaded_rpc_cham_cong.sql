-- =============================================================
-- 16_DROP_OVERLOADED_RPC_CHAM_CONG.SQL
-- Dọn dẹp lỗi trùng lặp tham số (varchar vs text) của hàm chấm công
-- =============================================================

-- Xoá sạch mọi phiên bản cũ
DROP FUNCTION IF EXISTS public.rpc_cham_cong(uuid, date, varchar, text);
DROP FUNCTION IF EXISTS public.rpc_cham_cong(uuid, date, text, text);
DROP FUNCTION IF EXISTS public.rpc_cham_cong(date, text, text);

-- Tạo lại bản chuẩn nhất với p_type_code là "text"
...
BEGIN
  ...
  IF v_existing_id IS NULL THEN
    ...
    RETURNING id INTO v_existing_id; -- Lấy ID vừa tạo ra để ghi log!
  ELSE
